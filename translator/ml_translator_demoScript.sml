...
val lookup_qsort = save_thm("lookup_qsort",
  EVAL ``nsLookup  ^(concl Decls_thm |> rator |> rand).v (Short (strlit "qsort"))``);
...
Theorem ML_QSORT_CORRECT:
   !env tys a ord R l xs refs.
      nsLookup env.v (Short (strlit "qsort")) = SOME qsort_v /\
      LIST_TYPE a l xs /\ (lookup_var (strlit "xs") env = SOME xs) /\
      (a --> a --> BOOL) ord R /\ (lookup_var (strlit "R") env = SOME R) /\
      transitive ord /\ total ord
      ==>
      ?l' xs' refs' ck1 ck2.
        evaluate (empty_state with <| clock := ck1; refs := refs |>) env
          [App Opapp [App Opapp [Var (Short (strlit "qsort"));
             Var (Short (strlit "R"))]; Var (Short (strlit "xs"))]] =
