...
/\
((evaluate:'ffi state ->(v)sem_env ->(exp)list -> 'ffi state#(((v)list),(v))result) st env [Tannot e t u]=
   ((case evaluate st env [u] of
    (_, Rerr (Rabort Rtype_error)) => (st, Rerr (Rabort Rtype_error))
  | _ => evaluate st env [e]
  )))
/\
