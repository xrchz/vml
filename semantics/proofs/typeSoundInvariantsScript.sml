...
val ctMap_has_lists_def = Define `
  ctMap_has_lists ctMap ⇔
    FLOOKUP ctMap (TypeStamp (Strlit "[]") list_type_num) = SOME ([Strlit "'a"],[],Tlist_num) ∧
    FLOOKUP ctMap (TypeStamp (Strlit "::") list_type_num) =
      SOME ([Strlit "'a"],[Tvar (Strlit "'a"); Tlist (Tvar (Strlit "'a"))],Tlist_num) ∧
    (!cn. cn ≠ (Strlit "::") ∧ cn ≠ (Strlit "[]") ⇒ FLOOKUP ctMap (TypeStamp cn list_type_num) = NONE)`;

(* The global constructor type environment has the bool primitives in it *)
val ctMap_has_bools_def = Define `
  ctMap_has_bools ctMap ⇔
    FLOOKUP ctMap (TypeStamp (Strlit "True") bool_type_num) = SOME ([],[],Tbool_num) ∧
    FLOOKUP ctMap (TypeStamp (Strlit "False") bool_type_num) = SOME ([],[],Tbool_num) ∧
    (!cn. cn ≠ (Strlit "True") ∧ cn ≠ (Strlit "False") ⇒ FLOOKUP ctMap (TypeStamp cn bool_type_num) = NONE)`;
